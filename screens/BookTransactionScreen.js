   ...
   return transactionType
    ...
    checkStudentEligibilityforBookIssue=async()=>{
      ...
      if(studentRef.docs.length==0){
        ...
        Alert.alert("The StudentId does not exsists in the database")
      }
      else{
    ...
    checkStudentEligibilityforBookReturn=async()=>{
      ...
      transactionRef.docs.map((docs)=>{
        var lastBookTransaction=docs.data();
        if(lastBookTransaction.studentId===this.state.scannedStudentId){
          isStudentEligible=true
        }
        else{
          isStudentEligible=false
          Alert.alert("The wasn't issued by this student")
          this.setState({
            scannedStudentId:'',
            scannedBookId:''
          })
    ...
    handleTransaction =async()=>{
      ...
      else if(transaction==="issue"){
        var isStudentEligible=await this.checkStudentEligibilityforBookIssue()
        if(isStudentEligible){
          this.initiateBookIssue(
            Alert.alert("Books issued to the student")
          )
        }
        else {
